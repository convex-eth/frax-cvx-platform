...
contract("Deploy System and test staking/rewards", async accounts => {
  it("should deploy contracts and test various functions", async () => {
    ...
    await cvxfxs.balanceOf(deployer).then(a=>console.log("balance of cvxfxs: " +a))
    await cvxfxs.totalSupply().then(a=>console.log("supply of cvxfxs: " +a))

    var minterdata = cvxfxs.contract.methods.setOperator(chainContracts.frax.bridge,true).encodeABI();
    console.log(minterdata);

    await booster.execute(cvxfxs.address, minterdata, {from:deployer});
    console.log("bridge given mint role")
    await cvxfxs.operators(chainContracts.frax.bridge).then(a=>console.log("bridge is operator? " +a));
    
    // await cvxfxs.mint(deployer, "123456789", {from:chainContracts.frax.bridge})
    // console.log("minted")
